...
def get_loader(experiment_name, client_data_path, batch_size, train):
    ...
    if experiment_name == "mnist":
        # Federated mode: per-client .npy files (generated by data/main.py)
        ...
        if os.path.isfile(npy_train):
            ...
            # Images are stored as (N, 28, 28) uint8 — normalise to float [0,1]
            data = data.reshape(len(data), -1).astype(np.float32) / 255.0
        ...
        dataset = CIFAR100(root=client_data_path, train=train, transform=transform)
...
def init_client(args, client_id, data_dir, logger, num_clients=None, threshold=5):
    """Initialize one SecureClient for federated learning.
    ...
    data_dir : str
        Path to this client's local data directory.
    logger :
        TensorBoard SummaryWriter for this client.
    num_clients : int, optional
        Total number of clients in the federation.
        If None, defaults to args.threshold (not ideal — always pass explicitly).
    threshold : int
        Minimum share count for SSS reconstruction.
...
def get_aggregator(aggregator_type, clients_dict, clients_weights_dict, global_trainer,
                   logger, verbose, seed, threshold=5):
    """
    Parameters
    ----------
    aggregator_type: str
    ...
    seed: int
    threshold: int
        Minimum shares required for SSS reconstruction (SecureAggregator only).
...
def get_local_optimums(clients_dict):
    ...
    for client_id, client in clients_dict.items():
        ...
        # Avoid crashing or printing warnings on image data (3D/4D tensors)
        if hasattr(train_dataset, "data") and hasattr(train_dataset.data, "ndim") and train_dataset.data.ndim > 2:
...
# (Deprecated helper functions removed — use init_client directly)
